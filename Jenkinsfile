...
stages {
	 ...
	 sh 'ANSIBLE_HOST_KEY_CHECKING=false ansible-playbook -i ansible/hosts ansible/Stage2_Install_Dependencies/tasks/main.yml'
         }
     }
	stage("Third Stage : Copy the code to the manager server"){ 
       steps{
         sh 'ANSIBLE_HOST_KEY_CHECKING=false ansible-playbook -i ansible/hosts ansible/Stage3_Copy_Code_to_Test_Server/tasks/main.yml'
         }
     }
